...
export const updateHistory: RequestHandler = async (req, res) => {
  ...
  //   const sameHistort = histories.find((item) => {
  //     if (item.audio.toString() === audio) return item;
  //   });
  const sameHistort = histories.find(
...
export const getHistories: RequestHandler = async (req, res) => {
  ...
  const histories = await History.aggregate([
    { $match: { owner: req.user.id } },
    {
      $project: {
        all: {
          $slice: ["$all", parseInt(pageNo) * parseInt(limit), parseInt(limit)],
        },
      },
    },
    {
...
export const getResentlyPlayed: RequestHandler = async (req, res) => {
